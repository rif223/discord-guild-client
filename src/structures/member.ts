...
export class Member {
   ...
   */
    ...
    kick(reason: string): Promise<void> {
        ...
        return this._client.rest.request(RestApi.HttpMethod.DELETE, apiEndpoints.guildMember(userId), { reason });
    }
     ...
     */
    ban(reason: string, options: { delete_message_days?: number, delete_message_seconds?: number } = {}) {
        ...
        const payload: any = {
            reason,
            delete_message_seconds: delete_message_seconds !== undefined
                ? delete_message_seconds
                : delete_message_days !== undefined
                    ? delete_message_days * 86400 // Convert days to seconds
    ...
    unban(userId: string, reason: string): Promise<void> {
        return this._client.rest.request(RestApi.HttpMethod.DELETE, apiEndpoints.guildBan(userId), { reason });
    }
    ...
    public async edit(options: {
        ...
        try {
            ...
            const data = await this._client.rest.request(
                ...
                apiEndpoints.guildMember(userId),
                payload
            );
    ...
    public async addRole(roleId: string): Promise<void> {
        ...
        try {
            ...
            await this._client.rest.request(
                ...
                apiEndpoints.guildMemberRole(this.user.id, roleId),
                {}
            );
        } catch (error) {
